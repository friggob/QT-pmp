...
cli::cli(Player *player, PlayList *list) {
	l = list;
	p = player;
	deleteFile = false;
	moveFile = false;
...
void cli::savePlaylist(QString fn,bool si) {
	...
	if(!l->savePlaylist(&fd,si)){
		qDebug() << "Could not save playlist!";
	}
}

void cli::printList()
{
	foreach(QString x,l->printList()){
		qStdout() << x << endl;
	}
}
...
void cli::run() {
	...
	while(!quiet && (buf = readline(ps1)) != NULL) {
		...
		if(b.startsWith('q')) {
			...
			if((b.count() > 2) && (b[1] == ' ')){
				qStdout() << "Saving playlist to " << b.mid(2).trimmed() << endl;
				this->savePlaylist(b.mid(2).trimmed(),false);
			} else {
				this->savePlaylist();
			}
			if(b[1] == 'q')
		...
		if(l->getIndex() >= l->getMaxIndex()) {
			qStdout() << "No more files to play! Bye bye!" << endl;
			break;
		}
